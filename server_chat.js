...
if( cluster.isMaster ){
	...
	const server =  https.createServer(credentials,app).listen(port, () => {
		...
		wss.on('connection', (ws, req) => {
			...
			var msg = CryptoJS.AES.encrypt(JSON.stringify({type:'new_user', text: 'Anonymous', id: ws.id, date: currentTime}), pass_enc).toString();
			ws.send(msg);
			...
			ws.on('message', message => {
				...
				if (messageParsed.type === 'private_msg') {
					...
					messageParsed.users = JSON.stringify([ws.id,ws.id==arr_ids[1]?arr_ids[0]:arr_ids[1]]);
					...
					// Send private chat message to toClient.
						...
						Object.assign(messageParsed, {with: {id: ws.id, username: ws.username, self: false}});
						var msg = CryptoJS.AES.encrypt(JSON.stringify(messageParsed), pass_enc).toString();
					...
					// Send private chat message to fromClient.
						...
						Object.assign(messageParsed, {with: {id: ws.id==arr_ids[1]?arr_ids[0]:arr_ids[1], username: messageParsed.username, self: true}});
						var msg = CryptoJS.AES.encrypt(JSON.stringify(messageParsed), pass_enc).toString();
				...
				// Public msg should be broadcasted.
					...
					ws.username = messageParsed.text+"("+(ws.upgradeReq.headers['x-forwarded-for'] || ws.upgradeReq.socket.remoteAddress)+")"+' '+process.pid;
				}
		...
		function broadCastThis(message) {
			var msg = CryptoJS.AES.encrypt(JSON.stringify(message), pass_enc).toString();
		...
		function connectToClient(fromId, toId) {
			...
			if (false && fromClient.readyState !== WebSocket.OPEN && toClient.readyState !== WebSocket.OPEN) {
				...
				var msg = CryptoJS.AES.encrypt(JSON.stringify({type: 'start_private_chat_failed'}), pass_enc).toString();
				if(fromClient){
					fromClient.send(msg);
					process.send( {pid: process.pid, msg: msg} );
				}
			...
			//else if (fromClient.readyState === WebSocket.OPEN && toClient.readyState === WebSocket.OPEN) {
				...
				let message = {type: 'start_private_chat', with: {id: fromId, username: fromClient?fromClient.username:""},users: strIds};
				var msg = CryptoJS.AES.encrypt(JSON.stringify(message), pass_enc).toString();
				if(Object.prototype.toString.call(toClient)==='[object Object]'){
					toClient.send(msg);
					process.send( {pid: process.pid, msg: msg} );
				...
				message = {type: 'start_private_chat', with: {id: toId, username: toClient?toClient.username:""},users: strIds};
				var msg = CryptoJS.AES.encrypt(JSON.stringify(message), pass_enc).toString();
				if(Object.prototype.toString.call(fromClient) ==='[object Object]'){
					fromClient.send(msg);
				...
				process.send( {pid: process.pid, msg: msg} );
				currentPrivateChat.push({user1Id: fromId, user2Id: toId});
			}
		}
