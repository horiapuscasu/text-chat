...
*/
	...
	const server =  https.createServer(credentials,app).listen(port, () => {
		...
		var msg = CryptoJS.AES.encrypt(punycode.toASCII(JSON.stringify({type:'new_user', text: 'Anonymous', id: ws.id, date: currentTime,usersp: JSON.stringify(["all-all"])})), pass_enc).toString();
		...
		function connectToClient(fromId, toId) {
			...
			if (false && fromClient.readyState !== WebSocket.OPEN && toClient.readyState !== WebSocket.OPEN) {
				...
				var msg = CryptoJS.AES.encrypt(punycode.toASCII(JSON.stringify({type: 'start_private_chat_failed'})), pass_enc).toString();
				if(fromClient){
					fromClient.send(msg);
					//process.send( {pid: process.pid, msg: msg} );
				}
