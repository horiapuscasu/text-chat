document.addEventListener('DOMContentLoaded', function() {
  ...
  document.body.addEventListener('click', e => {
    if (e.target &&
      ...
      e.preventDefault();
	  chattingWith = {id: e.target.getAttribute('data-id'), username: ""};
	 containerToWrite = document.getElementById("private-chat");
		...
		chattingWithUsers = [thisClientId,e.target.getAttribute('data-id')];
      //sendMessage('connect_private_chat', JSON.stringify([thisClientId,e.target.getAttribute('data-id')]));
      return false;
    }
  });
});
